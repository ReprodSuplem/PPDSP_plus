...
class PPDSP_MaxSAT_p4(PPDSP_reform):
	...
	def solve(self, verbose=1, time_limit=5):
		...
		cmd = f"stdbuf -oL uwrmaxsat -no-bin -no-sat -no-par -no-scip -ppdsp-time={time_limit} -ppdsp-lastY={lastY} -ppdsp={meta_file} -ppdsp-assume={assumption_file} {wcnf_file} | tee {log_file}"
