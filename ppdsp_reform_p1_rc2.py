...
class PPDSP_MaxSAT_p1(PPDSP_reform):
	...
	def resetVarIDforMaxSAT(self):
		if self.registry is not None:
			return
		self.varID = self.uVarList[0][0] - 1 # Reset to varID to 1st variable 'h^t_v'
	...
	def genMaxsatFormular(self):
		...
		#self.printUVarLits()
		self.vpool = IDPool(start_from = self.get_vpool_start_id()) # Setup vpool starting from varID+1 before running Eq.8-10
